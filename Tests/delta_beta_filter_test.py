...
class DeltaBetaFilterTest(unittest.TestCase):   
    def setUp(self):
        data = pd.DataFrame(
            {
                'CpG': [1, 2, 3],
    ...
    def test_calculate_delta_beta(self):
    ...
    def test_filter_delta_beta(self):
    ...
    def test_draw_volcano_plot(self):
        dbeta = self.dBetaFilter.CalculateDeltaBeta(self.data, 1)
        dbeta = self.dBetaFilter.FilterDeltaBeta(dbeta, self.dmp, False)
